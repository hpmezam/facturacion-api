...
def actualizarProducto(db: Session, id: int, producto: ProductoUpdate) -> ProductoRead:   
    existente = db.get(Producto, id)
    if not existente:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, 
            detail=f"Producto con id '{id}' no encontrado.")
    ...
    if producto.codigo_principal:
        existente = db.exec(
            select(Producto).where(
                Producto.codigo_principal == producto.codigo_principal,
                Producto.id != id
            )
        ).first()
        ...
        if existente:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Ya existe otro producto con el código principal: {producto.codigo_principal}"
            )
    
    # Validar relaciones (si se proporcionaron IDs)
    if producto.categoria_id is not None:
        categoria = db.get(CategoriaProducto, producto.categoria_id)
        if not categoria and producto.categoria_id != 0:  # 0 podría ser para quitar relación
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Categoría con ID {producto.categoria_id} no encontrada"
            )
    
    if producto.unidad_medida_id is not None:
        unidad_medida = db.get(UnidadMedida, producto.unidad_medida_id)
        if not unidad_medida:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Unidad de medida con ID {producto.unidad_medida_id} no encontrada"
            )
   
   
    datos_por_actualizar = producto.model_dump(exclude_unset=True)
    ...
    if no_cambios:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No se proporcionaron cambios para actualizar el producto.")
